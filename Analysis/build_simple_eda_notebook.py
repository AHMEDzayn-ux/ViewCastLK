"""Generate and run simple_eda.ipynb: a simple, explained EDA of every input.

Every chart is a count, a median or a percentage (helpers in simple_eda_lib.py).
The interpretation text in NOTES is written against the build named in
BUILD; the numbers inside the charts and tables are recomputed on every run.

Usage:
    python Analysis/build_simple_eda_notebook.py            # build and run
    python Analysis/build_simple_eda_notebook.py --no-run   # build only
"""
import os
import sys
...
NOTES = {}
if os.path.exists(os.path.join(HERE, "simple_eda_notes.py")):
    sys.path.insert(0, HERE)
    from simple_eda_notes import NOTES  # noqa: E402
...
import simple_eda_lib as L
from simple_eda_lib import H, short, times, say, save
...
path = os.path.join(HERE, "simple_eda.ipynb")
