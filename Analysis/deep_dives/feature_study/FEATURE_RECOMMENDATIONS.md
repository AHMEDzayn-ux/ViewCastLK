...
(`build_feature_study.py` in this folder, full numbers in `figures/feature_study_numbers.md`).
...
* **Fetch the channel's topics at forecast time.** In `prediction_api/app/youtube.py`,
  ...
  its own (14% without News & Politics; `Analysis/viewership_eda/viewership_eda.ipynb`, section 4),
...
* **Short or long-form: replace `is_short`, do not drop it.** The old flag was
  ...
  size (`Analysis/viewership_eda/viewership_eda.ipynb`, section 5, "Shorts against long-form by channel size").
