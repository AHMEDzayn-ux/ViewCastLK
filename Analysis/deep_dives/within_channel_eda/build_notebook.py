...
Usage:  cd Analysis/deep_dives/within_channel_eda && python build_notebook.py
"""
...
sys.path.insert(0, str(next(p for p in [Path.cwd(), *Path.cwd().parents] if (p / "paths.py").is_file())))
...
with open(Path(__file__).resolve().parent / "within_channel_eda.ipynb", "w", encoding="utf-8") as fh:
    nbf.write(nb, fh)
print(f"wrote within_channel_eda.ipynb ({len(C)} cells)")
